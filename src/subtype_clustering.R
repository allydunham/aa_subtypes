...
# Relabel outlier clusters to XO format from X0
relabel_outlier_clusters <- function(x){
  outliers <- x[str_detect(x, '^[A-Y]0$')]
  outliers <- str_replace(outliers, '0', CLUSTER_OUTLIER_CHAR)
  x[str_detect(x, '^[A-Y]0$')] <- outliers
  return(x)
}

# Sort cluster labels, with the assumption that 1-N are labeled appropriately (meaning by size in this work)
# Outliers are sorted after permissive
sort_clusters <- function(x){
  tibble(cluster = x) %>%
    tidyr::extract(cluster, c('wt', 'suffix'), str_c('^([A-Y])([0-9]*|', CLUSTER_OUTLIER_CHAR, '|', CLUSTER_PERMISSIVE_CHAR, ')$'), remove=FALSE) %>%
    mutate(num = suppressWarnings(as.integer(suffix)), # Can only be 0-9 or CLUSTER_OUTLIER_CHAR or CLUSTER_PERMISSIVE_CHAR, so suppressing is ok here, any other input has already been converted to NA
           num = str_replace(suffix, CLUSTER_PERMISSIVE_CHAR, as.character(max(num, na.rm = TRUE) + 1)) %>% 
             str_replace(CLUSTER_OUTLIER_CHAR, as.character(max(num, na.rm = TRUE) + 2)) %>% 
             as.integer()) %>%
    arrange(wt, num) %>%
    pull(cluster)
}
...
plot_cluster_diagnostics <- function(tbl, clusters, cols){
  cols <- enquo(cols)
