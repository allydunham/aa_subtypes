...
#### hclust clustering ####
make_hclust_clusters <- function(tbl, cols, h, min_size = 1, dist_method = 'euclidean', ...){
  cols <- enquo(cols)
...
make_cluster_plots <- function(tbl, cols, chem_env_cols){
  ...
  plots$cluster_sizes <- labeled_plot(plot_cluster_sizes(tbl), units='cm', height = 10, width = n_clusters*0.5 + 2)
  plots$mean_profiles <- labeled_plot(plot_cluster_profiles(tbl, !!cols), units='cm', height = n_clusters*0.5 + 2, width = 15)
  plots$profile_correlation <- labeled_plot(plot_cluster_profile_correlation(tbl, !!cols), units='cm', height = n_clusters*0.5 + 2, width = n_clusters*0.5 + 4)
  plots$foldx_profiles <- labeled_plot(plot_cluster_foldx_profiles(tbl), units='cm', height = n_clusters*0.5 + 2, width = 25)
  plots$chem_env_profiles <- labeled_plot(plot_cluster_chem_env_profiles(tbl, !!chem_env_cols), units='cm', height = n_clusters*0.5 + 2, width = 25)
  return(plots)
}
