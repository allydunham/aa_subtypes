...
make_cluster_plots <- function(tbl, cols, chem_env_cols){
  ...
  plots$cluster_sizes <- labeled_plot(plot_cluster_sizes(tbl), units='cm', height = 10, width = 14)
...
plot_cluster_chem_env_profiles <- function(tbl, cols){
  ...
  profiles <- cluster_chem_env_profiles(tbl, !!cols) %>%
    ...
    add_factor_order(cluster, aa, count) %>%
  ...
  max_val <- max(abs(profiles$rel_count))
