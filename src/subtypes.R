...
plot_tsne_clusters <- function(tbl){
  mutate(tbl, cluster_sym = str_sub(cluster, start = -1)) %>%
  ...
  ggplot(profiles,
    ...
    scale_y_continuous(breaks = 1:length(cluster_labs),
                       ...
                       sec.axis = sec_axis(~.,
                                           ...
                                           labels = prop_labs)) +
    scale_x_discrete(labels = sapply(FOLDX_TERMS_PLOTMATH, function(x){parse(text = x)})) +
    coord_fixed() +
...
full_cluster_characterisation <- function(tbl){
  ...
  foldx_profiles <- cluster_foldx_profiles(tbl) %>% 
    ...
    add_factor_order(cluster, term, rel_ddg) %>%
    mutate(rel_ddg = ifelse(n_foldx == 0, 0, rel_ddg),
...
plot_full_characterisation <- function(clusters, data, exclude_outliers=TRUE, global_scale=TRUE){
  ...
  if (length(cluster_order) == 1){
    # If only outliers exist just plot them
    exclude_outliers <- FALSE
    all_outliers <- TRUE
  }
  
  if (exclude_outliers){
    outliers <- cluster_order[str_ends(cluster_order, '0')]
  ...
  p_sizes <- filter(data$summary, cluster %in% clusters) %>%
