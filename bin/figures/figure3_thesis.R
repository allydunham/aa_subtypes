...
p_initial_profiles <- map(c('A', 'C', 'D', 'W', 'Y'), ~filter(dms, wt == .) %>%
                            ...
                            geom_raster() +
                            coord_fixed() +
                            scale_fill_distiller(type = ER_PROFILE_COLOURS$type, palette = ER_PROFILE_COLOURS$palette,
                                                 direction = ER_PROFILE_COLOURS$direction, limits = c(-1, 1)) +
                            guides(fill = FALSE) +
...
schematic_profiles <- pivot_wider(full_characterisation$profiles, cluster, names_from = 'mut', values_from = 'er') %>% 
...
p_schematic <- ggplot() +
  ...
  theme(axis.title = element_blank(),
        ...
        axis.ticks = element_blank(),
        panel.grid.major.y = element_blank()) +
  annotate('richtext', x = 0.5, y = 1.1, label = "<span style='font-size:11pt'>Clustering ER Profiles</span>", fill = NA, label.colour = NA) +
  
  # Initial profile heatmaps
  annotation_custom(ggplotGrob(p_initial_profiles$A), xmin = 0.14, xmax = 0.29, ymin = 0.8, ymax = 1) +
  annotate('richtext', x = 0.22, y = 1.01, label = "<span style='font-size:10pt'>A</span>", fill = NA, label.colour = NA) +
  annotation_custom(ggplotGrob(p_initial_profiles$C), xmin = 0.3, xmax = 0.45, ymin = 0.8, ymax = 1) +
  annotate('richtext', x = 0.38, y = 1.01, label = "<span style='font-size:10pt'>C</span>", fill = NA, label.colour = NA) +
  annotation_custom(ggplotGrob(p_initial_profiles$D), xmin = 0.46, xmax = 0.61, ymin = 0.8, ymax = 1) +
  annotate('richtext', x = 0.54, y = 1.01, label = "<span style='font-size:10pt'>D</span>", fill = NA, label.colour = NA) +
  annotation_custom(ggplotGrob(p_initial_profiles$W), xmin = 0.66, xmax = 0.81, ymin = 0.8, ymax = 1) +
  annotate('richtext', x = 0.74, y = 1.01, label = "<span style='font-size:10pt'>W</span>", fill = NA, label.colour = NA) +
  annotation_custom(ggplotGrob(p_initial_profiles$Y), xmin = 0.81, xmax = 0.96, ymin = 0.8, ymax = 1) +
  annotate('richtext', x = 0.89, y = 1.01, label = "<span style='font-size:10pt'>Y</span>", fill = NA, label.colour = NA) +
  ...
  annotate('richtext', x = 0.155, y = 0.7, label = "<span style='font-size:10pt'>AP</span>", fill = NA, label.colour = NA) +
  annotation_custom(ggplotGrob(p_a_profiles$Rest), xmin = 0.225, xmax = 0.375, ymin = 0.445, ymax = 0.745) +
  annotate('richtext', x = 0.305, y = 0.7, label = "<span style='font-size:10pt'>Rest</span>", fill = NA, label.colour = NA) +
  ...
  annotate('segment', x = 0.235, xend = 0.235, y = 0.2875, yend = 0.2125,
           arrow.fill = 'black', arrow = arrow(type = 'closed', length = unit(0.02, 'npc'))) +
  
  # Text
  annotate('TextBox', x = 0.405, y = 0.825, hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>1.<br></span>") +
  annotate('TextBox', x = 0.455, y = 0.825, width = unit(0.65, 'npc'), hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>Process each AA independantly<br></span>") +
  annotate('TextBox', x = 0.405, y = 0.68, hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>2.<br></span>") +
  annotate('TextBox', x = 0.455, y = 0.68, width = unit(0.65, 'npc'), hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>Split permissive positions (|ER|&nbsp;<&nbsp;0.4, labeled AP)<br></span>") +
  annotate('TextBox', x = 0.405, y = 0.5, hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>3.<br></span>") +
  annotate('TextBox', x = 0.455, y = 0.5, width = unit(0.65, 'npc'), hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>Heirarchical clustering using cosine distance between PC2:20 profiles<br></span>") +
  annotate('TextBox', x = 0.405, y = 0.25, hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>4.<br></span>") +
  annotate('TextBox', x = 0.455, y = 0.25, width = unit(0.65, 'npc'), hjust = 0, vjust = 1, fill = NA, box.colour = NA, lineheight = 0.6,
           label = "<span style='font-size:8.5pt'>Determines subtypes with hybrid dynamic tree cutting<br></span>")
...
prof_title <- function(x){
  str_c("<span style='font-size:14pt; color:", cor_set_colours[x],"'>&#9679;</span> ", x)
}
...
### Assemble figure part 1 ###
p1_1 <- p_schematic + labs(tag = 'A') + theme(text = element_text(size = 11))
p1_2 <- p_sizes + labs(tag = 'B') + theme(text = element_text(size = 9))
p1_3 <- p_heatmap + labs(tag = 'C') + theme(text = element_text(size = 11))

figure_1 <- multi_panel_figure(width = c(70, 70), height = c(70, 140), panel_label_type = 'none', row_spacing = 0, column_spacing = 0) %>%
  fill_panel(p1_1, row = 1, column = 1) %>%
  fill_panel(p1_2, row = 1, column = 2) %>%
  fill_panel(p1_3, row = 2, column = 1:2)

ggsave('figures/5_thesis/figure3_1.pdf', figure_1, width = figure_width(figure_1), height = figure_height(figure_1), units = 'mm',
       device = cairo_pdf)
ggsave('figures/5_thesis/figure3_1.png', figure_1, width = figure_width(figure_1), height = figure_height(figure_1), units = 'mm')

### Assemble figure part 2 ###
p2_1 <- p_umap + labs(tag = 'A') + theme(text = element_text(size = 12))
p2_2 <- p_subtype_freqs + labs(tag = 'B') + theme(text = element_text(size = 12))
p2_3 <- p_selective_profiles + labs(tag = 'C') + theme(text = element_text(size = 12))

figure_2 <- multi_panel_figure(width = c(90, 90), height = c(120, 90), panel_label_type = 'none', row_spacing = 0, column_spacing = 0) %>%
  fill_panel(p2_1, row = 1, column = 1:2) %>%
  fill_panel(p2_2, row = 2, column = 1) %>%
  fill_panel(p2_3, row = 2, column = 2)

ggsave('figures/5_thesis/figure3_2.pdf', figure_2, width = figure_width(figure_2), height = figure_height(figure_2), units = 'mm',
       device = cairo_pdf)
ggsave('figures/5_thesis/figure3_2.png', figure_2, width = figure_width(figure_2), height = figure_height(figure_2), units = 'mm')
