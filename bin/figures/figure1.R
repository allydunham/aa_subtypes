...
gene_summary <- group_by(dms, gene) %>%
  ...
  mutate(percent_struct = n_struct / n * 100,
         ...
         img = str_c("<img src='figures/4_figures/proteins/", gene_to_filename(gene), ".png", "' width='70' />"))
...
p_norm_trans <- filter(raw, study %in% c('steinberg_2016_tem1', 'heredia_2018_ccr5', 'matreyek_2018_tpmt')) %>%
  ...
  scale_x_continuous(breaks = c(0, -5), labels = c("<span style='font-size:6pt'><span style='line-height:4'>0</span><br>Neutral</span>", "<img src='figures/4_figures/parts/arrow.png' width='40' /><span style='font-size:6pt'><br>Deleterious</span>")) +
...
p_norm <- ggplot() +
  ...
  annotate('text', label = 'Process double\nmutants and\nmultiple mutations', x = 0.25, y = 1.85) +
  annotate('text', label = 'Transform scores\nto standard scale', x = 0.25, y = 1.45) +
  annotate('text', label = 'Normalise against\nmean of lowest\n10% of scores', x = 0.25, y = 1) +
  annotate('text', label = 'Filter positions\nwith scores for\n<16 substitutions', x = 0.25, y = 0.5) +
  annotate('richtext', label = '**Comparable<br>ER Scores**', x = 0.25, y = 0.15, fill = NA, label.colour = NA) +
...
figure1 <- multi_panel_figure(width = 300, height = 200, columns = 4, rows = 3,
                              panel_label_type = 'none', row_spacing = 0.1, column_spacing = 0.1) %>%
  fill_panel(p1, row = 1:2, column = 1:3) %>%
  fill_panel(p2, row = 1:2, column = 4) %>%
