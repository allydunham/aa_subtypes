...
avg_sil <- bind_rows(`Profile` = map(k, ~evaluate_k(dms, A:Y, ., min_size = 5)) %>% bind_rows(.id = 'k'),
                     ...
                     `PCA (profile distance)` = map(k, ~evaluate_k(dms, PC1:PC20, ., dist_cols = A:Y, min_size = 5)) %>% bind_rows(.id = 'k'),
                     `PCA (excluding PC1, profile distance)` = map(k, ~evaluate_k(dms, PC2:PC20, ., dist_cols = A:Y, min_size = 5)) %>% bind_rows(.id = 'k'),
                     .id = 'cols') %>%
