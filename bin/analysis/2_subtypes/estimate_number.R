...
plots$subtype_count_boxes_all <- group_by(counts, rep, positions) %>%
  summarise(nclusters = sum(nclusters), .groups = "drop") %>%
  ggplot(aes(x = positions, group = positions, y = nclusters)) +
  geom_boxplot() +
  labs(x = "Number of positions", y = "Functional Subtypes")
