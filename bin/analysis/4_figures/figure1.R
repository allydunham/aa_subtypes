...
gene_summary <- group_by(dms, gene) %>%
  ...
  mutate(percent_struct = n_struct / n * 100,
         img = str_c("<span style='font-size:7pt'>",
                     "<img src='figures/4_figures/proteins/", gene_to_filename(gene), ".png", "' width='20' />",
                     "<br>", n, " (", signif(percent_struct, 3), "%)</span>"))

p_genes <- ggplot(gene_summary, aes(x = n, y = n, label = img)) +
