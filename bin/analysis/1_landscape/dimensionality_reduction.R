#!/usr/bin/env Rscript
# Perform dimensionality reduction analyses on the full dataset
source('src/config.R')
...
#### tSNE/UMAP ####
tsne_plots <- tsne_umap_plots(dms_wide, tSNE1, tSNE2, 'tsne')
umap_plots <- rename(dms_wide, UMAP1=umap1, UMAP2=umap2) %>%
  tsne_umap_plots(UMAP1, UMAP2, 'umap')
plots <- c(plots, tsne_plots, umap_plots)
########
